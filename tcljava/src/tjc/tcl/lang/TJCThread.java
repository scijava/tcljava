    ...
    implements Runnable
{
    ...
    public static interface CompiledClassReady {
        public void compiled(
            final String geninfo,       // Name that identifies the source
    ...
    void compileJavaSource(
        String filename,
        String source,
        CompiledClassReady callback)
    {
        if (callback == null) {
    ...
    private
    void processEvent(Vector event) {
        int len = event.size();
        ...
        try {
            ...
            if (interp == null) {
                ...
                interp = new Interp();
                if (driver != null) {
                    interp.setVar("JAVA_DRIVER", null,
                        driver, 0);
                }
                interp.eval("source resource:/tjc/library/tjcthread.tcl");
            }
        ...
        for (int i=0; i < len; i += 2) {
            ...
            if (! (obj instanceof byte[])) {
                throw new TclException(interp,
                    "obj \"" + obj + "\" is not a byte[] instance");
            }
