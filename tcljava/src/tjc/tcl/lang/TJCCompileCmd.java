...
public void 
            ...
            throws TclException
    {
        ...
        TJCCompileJavaCmd cjobj = new TJCCompileJavaCmd(
            interp, javaInfo, readyCmd, readyVar);
	...
	if (proc == null) {
        ...
        TclList.append(interp, procList,
            TclString.newInstance("proc"));
        TclList.append(interp, procList,
            TclString.newInstance(cmd));
        ...
        for (i=0; i < len; i++) {
            ...
            if (defval == null) {
                TclList.append(interp, args,
                    TclString.newInstance(name.toString()));
            } else {
                TclObject defpair = TclList.newInstance();
                TclList.append(interp, defpair,
                    TclString.newInstance(name.toString()));
                TclList.append(interp, defpair,
                    TclString.newInstance(defval.toString()));
                TclList.append(interp, args, defpair);
            }
        }
        if (proc.isVarArgs) {
            TclList.append(interp, args,
                TclString.newInstance("args"));
        }
        TclList.append(interp, procList,
            TclString.newInstance(args));
        ...
        TclList.append(interp, procList,
            TclString.newInstance(proc.body.toString()));

        TJCCompileTclCmd ctobj = new TJCCompileTclCmd(
            interp, fullyQualifiedCmd, readyCmd, readyVar);

        if (ctobj.debug) {
            System.out.println("Sending proc decl\n" + procList.toString());
        }

        TJCThread.compileTclSource(
            javaClassName,
            procList.toString(),
            ctobj);
    }
    ...
    implements TJCThread.CompiledClassReady
{
    ...
    public void compiled(
        final String geninfo,       // Name of Tcl command that was compiled.
        ...
        final String msg)
    {
        ...
        if (cdata == null) {
            ...
            className = (String) cnames.get(0); // Name of class, could be null if not known
    ...
    public int processEvent(int flags) {
        ...
        if (!(o instanceof TJC.CompiledCommand)) {
            instErr =
                "instance of class " + className + " must extend TJC.CompiledCommand";
        }
        if (instErr != null) {
            status = false;
            errorMsg = instErr;
        ...
        if (readyVar != null) {
            ...
            if (status) {
                TclList.append(interp, tlist,
                    TclString.newInstance("OK"));
            } else {
                TclList.append(interp, tlist,
                    TclString.newInstance("FAIL"));
            }
            ...
            // MSG:
            TclList.append(interp, tlist,
                TclString.newInstance(errorMsg));
            ...
            // readyCmd
            TclList.append(interp, tlist,
                TclString.newInstance(readyCmd));
    ...
    void readyReport() {
        ...
        if (readyVar != null) {
            ...
            for (int i=0 ; i < cnames.size(); i++) {
                ...
                TclList.append(interp, cnames_list,
                    TclString.newInstance(cname));
            }
            }
