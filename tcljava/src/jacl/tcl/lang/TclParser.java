...
throws TclException
{
	...
	case PARSE_COUNT_NWLNE: {
	    ...
	    ParseCountNewline(interp, script, scriptLength,
		    objv[3], range2);
	    return;
	}
    }
        ...
        throws TclException
{
    ...
    TclList.append(interp, resultPtr,
	    ParseMakeByteRange(script, endCharIndex,
	        (charLength - (endCharIndex - charIndex))));
        ...
        throws TclException
{
	...
	TclList.append(interp, resultPtr,
                ParseMakeByteRange(script, charListOffset + elementIndex, size));
    }
 ...
 *	and raise a TclException. This method is invoked after something
 *	goes wrong in a parse operation.
 *
 * Results:
 *	None.
 *
 ...
 *	list object in the location referred to by result.
 *
 * Side effects:
 *	None.
 *
...
static int
ParseMakeTokenList(
    UTF8CharPointer script,		// Pointer to start of script being parsed.
        ...
        throws TclException
{
    ...
    TclList.append(null, resultList,
        ParseMakeByteRange(script, token.script_index, token.size));
 ...
 * ParseMakeRange --
 *
 *	Construct a new range object. This method depends on the
 *	script.index being set to the starting index of the
 *	entire script.
 *
 * Results:
...
static TclObject
ParseMakeRange(
    UTF8CharPointer script,	// Pointer to the start of whole script.
        ...
        throws TclException
{
    ...
    TclList.append(null, tlist, TclInteger.newInstance(
        start - scriptByteIndex ));
 ...
 * ParseMakeByteRange --
 *
 *	Construct a new range object containing a byte range given
 *	a start and length in characters.
 *
 * Results:
 *	Returns a newly allocated Tcl object.
 *
 ...
 * ParseGetString --
 *
 *	Extract the string from the script within the boundaries of
 ...
 * ParseCountNewline --
 *
 *	Count the number of newlines between a range of bytes
 ...
 *      is >= 0, set the value to the closest point. Note that
 *      indexes and ranges are in terms of bytes.
 *
 * Results:
 *	A standard Tcl result.
 *
 * Side effects:
 *	The values are written to the result argument.
...
ParseGetIndexAndLength(
    Interp interp,	    // Current interpreter.
    TclObject    rangePtr,
    int scriptLen,	    // Length of script in bytes. If >= 0, then try 
        ...
        try {
            ...
            if (bytes.length != bytesTotal) {
                throw new TclRuntimeError("generated " + bytes.length +
                        " but expected to generate " + bytesTotal +
                        " bytes");
            }
