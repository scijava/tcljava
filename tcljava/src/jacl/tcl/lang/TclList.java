...
public class TclList implements InternalRep {
	 ...
	 *                if tobj cannot be converted into a list.
	 */
	static final void append(Interp interp, TclObject tobj, TclObject[] objv,
			final int startIdx, final int endIdx) throws TclException {
		if (tobj.isShared()) {
			throw new TclRuntimeError(
					"TclList.append() called with shared object");
		}
	 ...
	 * @param valuePtr Tcl object to store in the designated list element
	 * 
	 *----------------------------------------------------------------------
	 * @throws TclException 
	 */
 ...
 * */
	 ...
	 * @param unique 
	 * 			  true if only the last set of duplicate elements found in the list have to be retained
	 * @param command
	...
	static void sort(Interp interp, TclObject tobj, int sortMode,
			int sortIndex, boolean sortIncreasing, boolean unique, String command)
			throws TclException {
		if (!tobj.isListType()) {
			setListFromAny(interp, tobj);
		}
