 ...
 *
 * Results:
 *	The return value is Tcl.OK if the command was parsed successfully
 ...
 *	parseObj is filled in with information about the expression that 
 *	was parsed.
 *
 * Side effects:
 *	If there is insufficient space in parseObj to hold all the
 *	information about the expression, then additional space is
 *	malloc-ed. If the procedure returns Tcl.OK then the caller must
...
//        }
    ...
    if (parse.result != TCL.OK) {
        throw new TclRuntimeError("non TCL.OK parse result in parseExpr(): " +
 ...
 *
 * Results:
 *	The return value is Tcl.OK on a successful parse and TCL_ERROR
...
static void
ParseCondExpr(Interp interp, ParseInfo info) throws TclException
...
static void
ParseLorExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, code;
    int srcStart;
...
static void
ParseLandExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, code;
...
static void
ParseBitOrExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, code;
    int srcStart, operator;
...
static void
ParseBitXorExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, code;
    int srcStart, operator;
...
static void
ParseBitAndExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, code;
    int srcStart, operator;
...
static void
ParseEqualityExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
...
static void
ParseRelationalExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
...
static void
ParseShiftExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, lexeme, code;
    int srcStart, operator;
...
static void
ParseAddExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, lexeme, code;
    int srcStart, operator;
...
static void
ParseMultiplyExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, lexeme, code;
    int srcStart, operator;
...
static void
ParseUnaryExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
    int firstIndex, lexeme, code;
    int srcStart, operator;
...
static void
ParsePrimaryExpr(Interp interp, ParseInfo info) throws TclException
{
    TclParse parseObj = info.parseObj;
	...
	stringStart = info.next;
        ...
        parseObj = Parser.ParseQuotedString(interp, info.originalExpr, (info.next - 1),
	...
	// '{' string '}'
        ...
        parseObj = Parser.ParseBraces(interp, info.originalExpr, info.start,
 ...
 *
 * Results:
 *	Tcl.OK is returned unless an error occurred. In that case a standard
 *	Tcl error code is returned and, if info.parseObj.interp is
 ...
 *	number. If the lexical analysis is successful, info.lexeme
 *	refers to the next symbol in the expression string, and
 *	info.next is advanced past the lexeme. Also, if the lexeme is a
 *	LITERAL or FUNC_NAME, then info.start is set to the first
...
static void
GetLexeme(Interp interp,ParseInfo info) throws TclException
    // info;		 Holds state needed to parse the expr, including the resulting lexeme. 
{
...
static void checkFuncName(Interp interp, ParseInfo info,int src ) {
    ...
    if (Character.isLetter(c)) {	// INTL: ISO only.
        ...
        while (Character.isLetterOrDigit(c) || (c == '_')) { // INTL: ISO only.
            src++;
            c = info.originalExpr[src];
        }
 ...
 *
 * Side effects:
 *	If there is insufficient space in parseObj to hold the new tokens,
...
static void
PrependSubExprTokens(int op, int opBytes, int src, int srcBytes, int firstIndex, ParseInfo info)
     /* op;			 Points to first byte of the operator
				 * in the source script. */
     /* opBytes;		 Number of bytes in the operator. */
     /* src;			/* Points to first byte of the subexpression
				 * in the source script. */
     /* srcBytes;		 Number of bytes in subexpression's
				 * source. */
     /* firstIndex;		 Index of first token already emitted for
				 * operator's first (or only) operand. */
     /* info;		/* Holds the parse state for the
...
// Extra info is disabled for now until the parser test cases are updated to
