 ...
 *  procedure raises a TclException. Otherwise a reference to a
...
ItclObject
CreateObject(
    Interp interp,           // interpreter mananging new object
 ...
 *  removing its access command, and this procedure returns normally.
 *  Otherwise, the object will remain alive, and this procedure
 *  raises a TclException.
 * ------------------------------------------------------------------------
 */
 ...
 *  by DeleteObject() or DestroyObject() as a part of the
 *  object destruction process.  If the ItclInt.IGNORE_ERRS flag is
 ...
 *
 *  Invoked by DestructObject() to recursively destruct an object
 ...
 *  classes.  If the ItclInt.IGNORE_ERRS flag is included, all destructors
 *  are invoked even if errors are encountered.
 *
 *  Raises a TclException if anything goes wrong.
 * ------------------------------------------------------------------------
 */
        ...
        throws TclException
{
    ...
    if (contextObj.destructed.get(contextClass.name) == null) {
        Methods.InvokeMethodIfExists(interp, "destructor",
            contextClass, contextObj, null);
    }
 ...
 *
 *  Raises a TclException if anything goes wrong. If an object
 *  was found, it is returned. Otherwise, null is returned.
 * ------------------------------------------------------------------------
 */
...
ItclObject
FindObject(
    Interp interp,           // interpreter containing this object
        ...
        throws TclException
{
    ...
    Util.DecodeScopedCommandResult res =
        Util.DecodeScopedCommand(interp, name);
    contextNs = res.rNS;
    cmdName = res.rCmd;
...
boolean
IsObject(
    WrappedCommand wcmd)         // command being tested
{
 ...
 *
 *  Return the ItclObject context object associated with a given
 *  This function assumes that IsObject() returns
...
GetContextFromObject(
    WrappedCommand wcmd)  // command that represents the object
{
 ...
 *  inheritance hierarchy.  Returns true if the object belongs to
...
{    
    ...
    context = new ItclContext(interp);
    Methods.PushContext(interp, null, contextObj.classDefn,
        contextObj, context);
 ...
 *
 *  Returns a String object summarizing the usage
        ...
        throws TclException
{
    ...
    if ((flags & TCL.TRACE_READS) != 0) {
        if (contextObj.accessCmd != null) {
            objName = interp.getCommandFullName(
 ...
 *
 *  Note that the usual way to delete an object is via DeleteObject().
    ...
    if (vdefn.init != null) {
        ...
        try {
            Methods.PushContext(interp, null,
                vdefn.member.classDefn, contextObj, context);
            interp.setVar(vdefn.member.fullname,
                TclString.newInstance(vdefn.init), 0);
        } catch (TclException ex) {
            // No-op
        } finally {
            Methods.PopContext(interp, context);
 ...
 *  desired variable in the object and returns the variable. If the
 *  variable does not start with "@itcl", this procedure returns
 *  null and variable resolution continues using the normal rules.
 *  If anything goes wrong, this procedure raises a TclException
 *  and variable access is denied.
 * ------------------------------------------------------------------------
 */
