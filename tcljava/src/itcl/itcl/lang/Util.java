 ...
 *
 *  Called to mimic an assert() statement in C. Raises a TclRuntimeError
...
void
InitList(
    Itcl_List list)     // list to be initialized
{
...
void
DeleteList(
    Itcl_List list)     // list to be deleted
{
    Itcl_ListElem elem;
 ...
 *
 *  Low-level routined used by procedures like InsertList() and
 *  AppendList() to create new list elements.  If elements are
...
Itcl_ListElem
CreateListElem(
    Itcl_List list)     // list that will contain this new element
{
    Itcl_ListElem elem;
...
Itcl_ListElem
DeleteListElem(
    Itcl_ListElem elem)     // list element to be deleted
{
    Itcl_List list;
    Itcl_ListElem next;
...
static
Itcl_ListElem
NextListElem(Itcl_ListElem elem) {
    return elem.next;
}
...
static
Itcl_ListElem
PrevListElem(Itcl_ListElem elem) {
    return elem.prev;
}
...
static
Object
GetListValue(Itcl_ListElem elem) {
    return elem.value;
}
...
void
PreserveData(
    ItclEventuallyFreed fobj)      // data to be preserved
{
...
void
ReleaseData(
    ItclEventuallyFreed fobj)      // data to be released
{
    ItclPreservedData chunk;
 ...
 *
 *  Converts an integer protection code (ITCL.PUBLIC, ITCL.PROTECTED,
 *  or ITCL.PRIVATE) into a human-readable character string.  Returns
 ...
 *
 *  Returns and object that contains a head and tail part.
 *  head part ("namesp::namesp::namesp"), tail part ("element")
 *  If the head part is missing, a the head member will be null
 *  and the rest of the string is returneed as the tail.
 * ------------------------------------------------------------------------
 */

static
ParseNamespPathResult
ParseNamespPath(
    String name)         // path name to class member
{
    int i;
    String head, tail;
 ...
 *  nothing and returns a null rNs and the passed in name
 *  value as the rCmd value. If the string is a scoped value
 *  then it is decoded, and the namespace, and the simple command
 *  string are returned. If anything goes wrong, this procedure
        ...
        throws TclException
{
	...
	if (((pos + 8) <= len) && (name.charAt(pos) == 'i') &&
            ...
            if (ex == null) {
                if (listv.length != 4) {
                    // Create exception, then add error info below before throwing
                    ex = new TclException(interp, "malformed command \"" + name +
                        "\": should be \"" +
                        "namespace inscope namesp command\"");
                } else {
 ...
 *  context.
 *
 *  Returns if successful. Otherwise, this procedure raises
 *  a TclException.
 * ------------------------------------------------------------------------
 */
 ...
 *
 *  This procedure takes a string and a list of objv arguments,
...
static
void
StartSublist(StringBuffer buffer)
{
    if (NeedSpace(buffer)) {
        buffer.append(" {");
    } else {
...
static
void
EndSublist(StringBuffer buffer)
{
    buffer.append('}');
}
...
static
void
AppendElement(StringBuffer buffer, String elem)
{
    if (NeedSpace(buffer)) {
        buffer.append(' ');
    }
