...
static class BodyCmd implements Command {
public void
cmdProc(
...
static class ConfigBodyCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
 ...
 *  If another command with the same name is already installed, then
 *  it is overwritten. Returns if successful, raises TclException if
 ...
 *  a Java procedure registered by Itcl_RegisterC().
 *
 ...
 *  for a Java procedure registered by Itcl_RegisterC().
 *
 *  Returns if successful, raises TclException if something goes wrong.
 * ------------------------------------------------------------------------
 */
...
void
DeleteMemberFunc(
    ItclMemberFunc mfunc)  // ref to member function definition
{
    if (mfunc != null) {
...
static
boolean
IsMemberCodeImplemented(ItclMemberCode mcode) {
 ...
 *  Invoked automatically by Util.ReleaseData() when the implementation
...
void
DeleteMemberCode(
    ItclMemberCode mcode)                    // ref to member function definition
{
    if (mcode.arglist != null) {
 ...
 *  implementation is not defined and cannot be autoloaded.
 *  Returns if implementation is ready to use.
 * ------------------------------------------------------------------------
 */
 ...
 *  fragment.  This code may be a body of Tcl commands, or a
 *  Java handler procedure.
 *
        ...
        throws TclException
{
    ...
    for (int i = Util.GetStackSize(info.transparentFrames)-1; i >= 0; i--) {
        transFrame = (CallFrame)
            Util.GetStackValue(info.transparentFrames, i);
    ...
    if (mfunc != null && objv.length > 0) {
        if ((mcode.flags & ItclInt.IMPLEMENT_TCL) != 0 ||
            ( (member.flags & ItclInt.CONSTRUCTOR) != 0 &&
...
CreateArgListResult
CreateArgList(
    Interp interp,            // interpreter managing this function
...
static class CreateArgListResult {
    int argcount;
 ...
 *
 *  Returns a TclObject containing the string representation for the
 *  given argument list.
 * ------------------------------------------------------------------------
 */
    ...
    int arg2c)             // number of args in matching list
{
    ...
    while (arg1 != null && arg1c > 0 && arg2 != null && arg2c > 0) {
        ...
        arg1 = arg1.next;  arg1c--;
        arg2 = arg2.next;  arg2c--;
    }
    if (arg1c == 1 && arg1.name.equals("args")) {
        return true;
    }
 ...
 *
 *  Returns the string by appending it onto the TclObject passed in as
 *  an argument.
 * ------------------------------------------------------------------------
 */
    ...
    StringBuffer buffer)        // returns: string showing usage
{
    ...
    if ((mfunc.member.flags & ItclInt.COMMON) == 0) {
        if ((mfunc.member.flags & ItclInt.CONSTRUCTOR) != 0 &&
            ...
            if (mf == mfunc) {
                String fname =
                    contextObj.classDefn.interp.getCommandFullName(
                        contextObj.classDefn.w_accessCmd);
                buffer.append(fname);
                buffer.append(" ");
                name =
                    contextObj.classDefn.interp.getCommandName(
            ...
            name = contextObj.classDefn.interp.getCommandName(
                contextObj.w_accessCmd);
            buffer.append(name);
            buffer.append(" ");
 ...
 *  scope of the class where it was defined.  For Java procedures, the
 *  arguments are passed in "as-is", and the procedure is executed in
 *  the most-specific class scope.
 * ------------------------------------------------------------------------
 */

static class ExecMethod implements CommandWithDispose {
    ...
    TclException
{
    ...
    if (mfunc.member.protection != Itcl.PUBLIC) {
        ...
        if (!Util.CanAccessFunc(mfunc, contextNs)) {
            throw new TclException(interp,
                ...
                Util.ProtectionStr(member.protection) + " function");
        }
    }
    ...
    if (token.indexOf("::") == -1) {
        ItclMemberFunc tmp = (ItclMemberFunc) contextObj.classDefn.resolveCmds.get(
            member.name);
        if (tmp != null) {
            mfunc = tmp;
            member = mfunc.member;
    ...
    try {
        Methods.EvalMemberCode(interp, mfunc, member, contextObj,
            objv);
    } catch (TclException ex) {
...
static class ExecProc implements CommandWithDispose {
    ItclMemberFunc mfunc;
    ...
    TclException
{
    ...
    try {
        Methods.EvalMemberCode(interp, mfunc, member,
            null, objv);
    } catch (TclException ex) {
...
void
PushContext(
    Interp interp,            // interpreter managing this body of code
    ItclMember member,        // member containing code body
        ...
        throws TclException
{
    ...
    if (member != null) {
        ...
        //localCt = mcode.argcount; // Jacl Procedure has no numCompiledLocals
 ...
 *
 *  Removes a class/object context previously set up by PushContext.
 ...
 *  Useful in implementing methods/procs to see what class or
 *  what object, is active.
 *
 *  Returns the current class and or object.
 *  Raises a TclException if a class namespace is not active.
 * ------------------------------------------------------------------------
 */
        ...
        throws TclException
{
    ...
    throw new TclException(interp,
        "namespace \"" + activeNs.fullName +
        "\" is not a class namespace");
}

static class GetContextResult {
    ItclClass cdefn;
    ItclObject odefn;
        ...
        throws TclException
{
    ...
    frame = Migrate.GetCallFrame(interp, 0);
    ItclAccess.setCallFrameObjv(frame, objv); // ref counts for args are incremented below
    ...
    for (argsLeft = mcode.argcount, arg = mcode.arglist, objvi = 1, objc = objv.length - 1;
         argsLeft > 0;
         arg = arg.next, argsLeft--, /*varPtr++,*/ objvi++, objc--)
    {
                  ...
                  contextObj != null )
        {
            ...
            if (objc > 0) {  // still have some arguments left?
                ...
                for (vi=0; vi < configc; vi++) {
                    StringBuffer buffer = new StringBuffer(64);
                    buffer.append(
                        configVars[vi].member.classDefn.name);
                    buffer.append("::");
                    buffer.append(configVars[vi].member.name);
 ...
 *  Methods.AssignLocal
 *
 *  Assign a local variable by adding a Var to the varTable
 *  for the current frame. This is needed so that we don't
 *  accidently overwrite common or instance vars while setting
 *  the values/defaults for arguments.
 * ------------------------------------------------------------------------
 */
 ...
 *  correctly.  Returns an objects containing the number of
 *  variables accessed, an array of public variable names, and
 *  their corresponding values. These values are passed to
        ...
        throws TclException
{
    ...
    while (objc-- > 0) {
        ...
        vlookup = (ItclVarLookup) contextObj.classDefn.resolveVars.get(
            varName.substring(1));
 ...
 *  The list of assignments is parsed in ParseConfig(), but the
 ...
 *  variable is set back to its previous value and an exception is raised.
 *
        ...
        throws TclException
{
    ...
    context = new ItclContext(interp);
    Methods.PushContext(interp, null,
    ...
    for (int i=0; i < argc; i++) {
        ...
        try {
            ...
            interp.setVar(vars[i].member.fullname,
                TclString.newInstance(vals[i]), 0);
        } catch (TclException ex) {
            interp.addErrorInfo(
        ...
        if (vars[i].member.code != null) {
            ...
            if (evalEx != null) {
                ...
                interp.setVar(vars[i].member.fullname,
                    TclString.newInstance(lastval.toString()), 0);
 ...
 *  this frame.  Raises a TclException if anything goes wrong.
 * ------------------------------------------------------------------------
 */
        ...
        throws TclException
{
    ...
    while (elem != null) {
        ...
        if (contextObj.constructed.get(cdefn.name) == null) {
            ...
            Methods.InvokeMethodIfExists(interp, "constructor",
                cdefn, contextObj, null);
        ...
        throws TclException
{
    ...
    if (mfunc != null) {
        ...
        try {
            EvalMemberCode(interp, mfunc, member,
                contextObj, cmdlinev);
        } catch (TclException ex) {
    ...
    if (exp != null) {
        ...
        if (code == TCL.RETURN) {
            ...
            throw new TclRuntimeError(
                "unexpected TclException completion code : " +
                code);
        }
        buffer = new StringBuffer(64);
        buffer.append("\n    ");
