        ...
        throws TclException
{
    ...
    interp.setAssocData(ItclInt.INTERP_DATA,
        info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::find",
        "classes", "?pattern?",
        new FindClassesCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::delete",
        "class", "name ?name...?",
        new DelClassCmd(info));
    Util.PreserveData(info);

    Ensemble.AddEnsemblePart(interp, "::itcl::delete",
        "object", "name ?name...?",
        new DelObjectCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::is",
        "class", "name",
        new IsClassCmd(info));
    Util.PreserveData(info);

    Ensemble.AddEnsemblePart(interp, "::itcl::is",
        "object", "?-class classname? name",
        new IsObjectCmd(info));
    Util.PreserveData(info);
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::import::stub",
        "create", "name",
        new StubCreateCmd());
    Ensemble.AddEnsemblePart(interp, "::itcl::import::stub",
        "exists", "name",
    ...
    interp.setVar("::itcl::patchLevel",
        TclString.newInstance(Itcl.PATCH_LEVEL),
        TCL.NAMESPACE_ONLY);

    interp.setVar("::itcl::version",
        TclString.newInstance(Itcl.VERSION),
        TCL.NAMESPACE_ONLY);
...
void
DelObjectInfo(
    ItclObjectInfo info)    // client data for class command
{
...
static class FindClassesCmd implements CommandWithDispose {
    ItclObjectInfo info;
    ...
    while (Util.GetStackSize(search) > 0) {
        ...
        for (Enumeration e = ns.cmdTable.keys(); e.hasMoreElements() ; ) {
            ...
            if (Class.IsClass(cmd)) {
                ...
                if (newEntry &&
			(pattern == null ||
                        tcl.lang.Util.stringMatch(cmdName, pattern))) {
        ...
        handledActiveNs = true;  // don't process the active namespace twice
...
static class FindObjectsCmd implements CommandWithDispose {
    ItclObjectInfo info;
			...
			(pattern == null ||
                    ...
                    if (classDefn == null ||
                        ...
                        if (isaDefn == null) {
                            ...
                            if (contextObj.classDefn.heritage.get(isaDefn)
                                    != null) {
                                match = true;
                            }
                        }
...
static class ProtectionCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ScopeCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
    ...
    TclException
{
    ...
    if (Class.IsClassNamespace(contextNs)) {
        ...
        TclList.append(interp, list,
            TclString.newInstance("@itcl"));

        TclList.append(interp, list,
            TclString.newInstance(
                interp.getCommandFullName(contextObj.w_accessCmd)));
        ...
        TclList.append(interp, list,
            TclString.newInstance(buffer.toString()));
...
static class StubCreateCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class StubExistsCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
 ...
 *  stub created by the "stub create" command.  Returns true if
 ...
 *  command to handle its function.
 *  If successful, this command will set the interpreter result
 *  with the result from the real implementation.
 *  Will raise a TclException to indicate failure.
 * ------------------------------------------------------------------------
 */

static class HandleStubCmd implements CommandWithDispose {
    WrappedCommand wcmd;
...
static class IsObjectCmd implements CommandWithDispose {
    ItclObjectInfo info;
 ...
 * ------------------------------------------------------------------------
 */
static class IsClassCmd implements CommandWithDispose {
    ItclObjectInfo info;
