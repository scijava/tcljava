 ...
 *  will raise a TclException to indicate failure. If successful,
 *  a reference to a new class definition will be returned.
 * ------------------------------------------------------------------------
 */
...
ItclClass
CreateClass(
    Interp interp,		// interpreter that will contain new class
        ...
        throws TclException
{
    ...
    try {
        vdefn = CreateVarDefn(interp, cd, "this",
            null, null);
...
void
DestroyClass(
    ItclClass cdefn)  // class definition to be destroyed
{
...
static
void
DestroyClassNamesp(ItclClass cdefn)
{
    ItclObject contextObj;
 ...
 *  [incr Tcl] class.  Returns true if so, and false otherwise.
 * ------------------------------------------------------------------------
 */
 ...
 *
 *  Return the ItclClass associated with a given class namespace.
 *  This function assumes that IsClassNamespace() returns
 *  true for this namespace.
 * ------------------------------------------------------------------------
 */
 ...
 *  definition.  Otherwise, if the autoload flag is true, an
 *  attempt will be made to autoload the class definition.  If it
 *  still can't be found, this procedure returns null.
 * ------------------------------------------------------------------------
 */
 ...
 *
 *  This procedure returns a reference to the desired namespace, or
    ...
    TclException
{
    ...
    if (start.indexOf("#auto") != -1) {
        ...
        while (true) {
            ...
            // Check for any commands with the given name, not just objects.
 ...
 *  and procs, and returns the WrappedCommand if found. If a command
 *  is private a TclException will be raised and access to the command
 *  is denied.  If a command is not recognized, this procedure returns
 *  null and the lookup continues via the normal Tcl name resolution
 *  rules.
 * ------------------------------------------------------------------------
 */
        ...
        throws TclException
{
    ...
    if (mfunc.member.protection != Itcl.PUBLIC) {
        ...
        if (!Util.CanAccessFunc(mfunc, context)) {
            ...
            // FIXME: The above says variable, but it should be command, right?
 ...
 *  common variables and instance variables at runtime.
 *
 *  If a variable is found, this procedure returns it.  If a particular
 *  variable is private, this procedure raises a TclException and
 *  access to the variable is denied.  If a variable is not recognized,
 *  this procedure returns null and lookup continues via the normal
        ...
        throws TclException
{
    ...
    if (varFrame != null &&
            ItclAccess.isProcCallFrame(varFrame) &&
            name.indexOf("::") == -1) {
    ...
    if (contextObj.classDefn != vlookup.vdefn.member.classDefn) {
        ItclVarLookup tmp = (ItclVarLookup)
            contextObj.classDefn.resolveVars.get(
                vlookup.vdefn.member.fullname);
        if (tmp != null) {
            vlookup = tmp;
        }
...
void
BuildVirtualTables(
    ItclClass cdefn)       // class definition being updated
{
    ItclVarLookup vlookup;
    ItclVarDefn vdefn;
    ItclMemberFunc mfunc;
    ...
    while (cd != null) {
        ...
        while (e.hasMoreElements()) {
            ...
            vlookup.accessible =
                ( vdefn.member.protection != Itcl.PRIVATE ||
                  vdefn.member.classDefn == cdefn );
 ...
 *
 *  Raises a TclException if anything goes wrong. Otherwise, returns
 *  a reference to a new variable definition.
 * ------------------------------------------------------------------------
 */
...
ItclVarDefn
CreateVarDefn(
    Interp interp,       // interpreter managing this transaction
 ...
 *
 *  Destroys a variable definition created by CreateVarDefn(),
 ...
 *  Returns a reference to the new representation.
 * ------------------------------------------------------------------------
 */
...
void
DeleteMember(
    ItclMember mem)  // pointer to member function definition
{
    if (mem != null) {
        mem.name = null;
        mem.fullname = null;
