 ...
 *  definitions. Will raise a TclException if anything goes wrong.
 * ------------------------------------------------------------------------
 */
        ...
        throws TclException
{
    ...
    interp.createCommand("::itcl::parser::inherit",
        new ClassInheritCmd());
    ...
    interp.createCommand("::itcl::parser::method",
        new ClassMethodCmd());

    interp.createCommand("::itcl::parser::proc",
        new ClassProcCmd());

    interp.createCommand("::itcl::parser::common",
        new ClassCommonCmd());

    interp.createCommand("::itcl::parser::variable",
    ...
    interp.createCommand("::itcl::parser::public",
    ...
    interp.createCommand("::itcl::parser::private",
        new ClassProtectionCmd(pInfo));
...
static class ClassCmd implements CommandWithDispose {
    ItclObjectInfo info;
...
static class ClassInheritCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
{
    ItclObjectInfo info =
        (ItclObjectInfo) interp.getAssocData(ItclInt.INTERP_DATA);
    ...
    for (int i = 1; i < objv.length ; i++) {
        ...
        if (baseCdefn == cdefn) {
            //goto inheritError;
            ClassInheritCmdInheritError(interp, cdefn,
                "class \"" + cdefn.name + "\" cannot inherit from itself");
        }
...
static
void
ClassInheritCmdInheritError(Interp interp, ItclClass cdefn, String exmsg)
        throws TclException
{
    Itcl_ListElem elem;
...
static class ClassProtectionCmd implements CommandWithDispose {
ProtectionCmdInfo pInfo;
    ...
    TclException
{
    ...
    // Removed TCL_BREAK, TCL_CONTINUE error since eval() raises them

    } catch (TclException ex) {
        interp.addErrorInfo(
            "\n    (" + objv[0].toString() + " body line "
            + interp.getErrorLine() + ")");
    } finally {
...
static class ClassConstructorCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ClassDestructorCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ClassMethodCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ClassProcCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ClassVariableCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
...
static class ClassCommonCmd implements Command {
public void
cmdProc(
    Interp interp,  			// Current interp.
    TclObject[] objv)			// Args passed to the command.
    ...
    TclException
{
    ...
    var = Migrate.NewVar();
    ItclAccess.createCommonVar(var, vdefn.member.name,
        ns, ns.varTable);
...
static class ParseVarResolverImpl implements Resolver {
...
static
void
FreeParserCommandData(
    Object cdata)  // client data to be destroyed
{
    //ckfree(cdata);
}
