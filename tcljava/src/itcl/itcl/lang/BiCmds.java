...
private static BiMethod[] BiMethodList = {
    new BiMethod("cget",
                   "-option",
                   "@itcl-builtin-cget", new BiCgetCmd()),
    new BiMethod("configure",
                   "?-option? ?value -option value...?",
                   "@itcl-builtin-configure", new BiConfigureCmd()),
    new BiMethod("isa",
                   "className",
                   "@itcl-builtin-isa", new BiIsaCmd())
};
        ...
        throws TclException
{
    ...
    for (int i=0; i < BiMethodListLen; i++) {
        Linkage.RegisterObjC(interp,
            BiMethodList[i].registration.substring(1),
            BiMethodList[i].proc);
    }
    ...
    Ensemble.AddEnsemblePart(interp, "::itcl::builtin::info",
            "class", "",
            new BiInfoClassCmd());
 ...
 *  in its heritage. Set the interpreter result to 1 if so,
 *  and to 0 otherwise.
 * ------------------------------------------------------------------------
 */
    ...
    TclException
{
    ...
    //  HANDLE:  configure -option
        ...
        vlookup = (ItclVarLookup)
            contextClass.resolveVars.get(token.substring(1));
        if (vlookup != null) {
    ...
    for (int i=1; i < objv.length; i+=2) {
        ...
        try {
            interp.setVar(member.fullname,
                TclString.newInstance(token), 0);
        ...
        if (mcode != null && Methods.IsMemberCodeImplemented(mcode)) {
            ...
            try {
                ...
                interp.setVar(member.fullname,
                    TclString.newInstance(buffer.toString()), 0);
 ...
 *  Returns a TclObject containing the information.
 * ------------------------------------------------------------------------
 */
...
TclObject
ReportPublicOpt(
    Interp interp,          // interpreter containing the object
    ...
    ItclObject contextObj)  // object containing this variable
{
    ...
    vlookup = (ItclVarLookup) cdefn.resolveVars.get(
        vdefn.member.fullname);
    Util.Assert(vlookup != null,
               "vlookup != null");
 ...
 *  a specific command is returned. Raises a TclException if
 *  anything goes wrong.
 * ------------------------------------------------------------------------
 */
    ...
    if (cmdName != null) {
        ...
        //  figure out what to return.
            ...
            for (int i=0 ; i < objc; i++) {
                iflist[i] =
                    TclIndex.get(interp, objv[i+skip], options, "option", 0);
            }
        }
            ...
            switch (iflist[i]) {
                case BIfArgsIdx:
                    if (mcode != null && mcode.arglist != null) {
                        obj = Methods.ArgList(mcode.argcount,
                                              mcode.arglist);
                    }
                    else if ((mfunc.member.flags & ItclInt.ARG_SPEC) != 0) {
                        obj = Methods.ArgList(mfunc.argcount,
                                              mfunc.arglist);
                    }
                    else {
                        obj = TclString.newInstance("<undefined>");
                ...
                case BIfTypeIdx:
                    val = ((mfunc.member.flags & ItclInt.COMMON) != 0)
                        ? "proc" : "method";
                    obj = TclString.newInstance(val);
                    break;
            }
 ...
 *  specific member is returned.  Raises a TclException if
 *  anything goes wrong.
 * ------------------------------------------------------------------------
 */
    ...
    if (varName != null) {
        ...
        for (int i=0 ; i < objc; i++) {
            switch (ivlist[i]) {
                ...
                case BIvTypeIdx:
                    val = ((member.flags & ItclInt.COMMON) != 0)
                        ? "common" : "variable";
                    obj = TclString.newInstance(val);
                    break;
                ...
                case BIvValueIdx:
                    if ((member.flags & ItclInt.COMMON) != 0) {
                        ...
                        val = Objects.GetInstanceVar(interp, member.fullname,
