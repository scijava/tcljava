 ...
 *
 *     RCS:  $Id: Ensemble.java,v 1.5 2009/09/16 21:49:18 mdejong Exp $
 * ========================================================================
        ...
        throws TclException
{
    ...
    for (int i=0; i < nameArgc; i++) {
        TclList.append(
            interp,
            list,
            TclString.newInstance(nameArgv[i]));
    }
...
void
EnsembleInit(
    Interp interp)         // interpreter being initialized
{
        ...
        throws TclException
{
    ...
    try {
        CreateEnsemble(interp, parentEnsData,
            nameArgv[nameArgv.length-1]);
    } catch (TclException ex) {
        CreateEnsembleFailed(interp, ensName, ex);
    }
}
 ...
 *      to the buffer argument and the function returns true.
 *      If anything goes wrong, this procedure returns false;
 *
 * Side effects:
 *      Buffer passed in is modified. 
 *
 *----------------------------------------------------------------------
 */
...
boolean
GetEnsembleUsage(
    Interp interp,    // interpreter containing the ensemble
 ...
 * Results:
 *      If the ensemble is found, its usage information is appended
 *      onto the buffer, and this procedure returns true.
 *      It is the responsibility of the caller to init the buffer.
 *      If anything goes wrong, this procedure returns false.
 *
 * Side effects:
 *      Buffer passed in is modified.
 ...
 *
 * Results:
 *      Appends usage information onto the bufer.
 *
 * Side effects:
 *      None.
 *
 ...
 *
 * Results:
 *      Returns usage information in the buffer.
 *
 * Side effects:
 *      None.
 *
    ...
    StringBuffer buffer)    // returns: usage information
{
    ...
    else if (ensPart.cmd != null &&
             (ensPart.cmd instanceof HandleEnsemble)) {
 ...
 *
 * Results:
 *      If anything goes wrong, this procedure raises a TclException
 *
 * Side effects:
 *      None.
 *
...
static
void
DeleteEnsemble(Ensemble ensData)
{
 ...
 *
 * Results:
 *      If the ensemble name is invalid then null will be returned.
 *      If anything goes wrong, this procedure raises a TclException
 *
 * Side effects:
 *      None.
 ...
 *      raises a TclException. If the part can be found, returns a
 *      reference to the part. Otherwise, it returns NULL.
 *
 * Side effects:
 *      None.
 *
 ...
 *      a status of true, along with the index of the part in pos.
 *      Otherwise, it returns a status of false, along with an
 *      index in pos indicating where the part should be.
 *
 * Side effects:
 *      None.
 *
 ...
 *
 * Results:
 *      Returns a reference to the ensemble parser data structure.
 *
 * Side effects:
    ...
    Interp interp)     // interpreter handling the ensemble
{
    ...
    ensInfo = (EnsembleParser) interp.getAssocData(
        "itcl_ensembleParser");
    ...
    ensInfo.parser.createCommand("part",
        new EnsPartCmd(ensInfo));

    ensInfo.parser.createCommand("option",
        new EnsPartCmd(ensInfo));

    ensInfo.parser.createCommand("ensemble",
        new EnsembleCmd(ensInfo));
...
void
DeleteEnsParser(
    EnsembleParser ensInfo,   // parser for ensemble-related commands
    ...
    TclException
{
    ...
    proc = ItclAccess.newProcedure(interp,
        wcmd.ns,
        partName,
        objv[2],
        objv[3],
    ...
    ensPart = AddEnsemblePart(interp, ensData, partName, usage,
        proc);
 ...
 *
 * Side effects:
 *      Raises a TclException with an error message.
 *
 *----------------------------------------------------------------------
 */
...
void
EnsembleErrorCmd(
    Ensemble ensData, // like client data in C version
    Interp interp,
    TclObject[] objv,
    int skip)
        throws TclException
...
static
void
FreeEnsInvocInternalRep(ItclEnsInvoc obj)
{
 ...
 *
 * Side effects:
 *      returns copy of internal rep with duplicates of the objects
...
DupEnsInvocInternalRep(
    ItclEnsInvoc obj)                   // internal rep to copy.
 ...
 *
 * Results:
 *      Returns the full command name for the ensemble part.
 *
 *----------------------------------------------------------------------
 */
